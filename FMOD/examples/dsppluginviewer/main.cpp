...
#define TITLE  "DSP Plugin Viewer Example. Copyright (c), Firelight Technologies Pty, Ltd 2004-2010."
    ...
    */
    {
        ...
        SetWindowLongPtr(plugin->hwnd, GWLP_USERDATA, (LONG_PTR)plugin);
        plugin->oldpluginproc = (WNDPROC)GetWindowLongPtr(plugin->hwnd, GWLP_WNDPROC);
        SetWindowLongPtr(plugin->hwnd, GWLP_WNDPROC, (LONG_PTR)FMOD_PluginInstanceProc);
        ...
        */
        for (count2 = 0; count2 < plugin->numparams; count2++)
        {
            ...
            */
            SetWindowLongPtr(param->sliderhwnd, GWLP_USERDATA, (LONG_PTR)param);
            param->oldtrackbarproc = (WNDPROC)GetWindowLongPtr(param->sliderhwnd, GWLP_WNDPROC);
            SetWindowLongPtr(param->sliderhwnd, GWLP_WNDPROC,  (LONG_PTR)FMOD_TrackbarProc);
...
LRESULT CALLBACK FMOD_PluginInstanceProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    Plugin *plugin = (Plugin *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
LRESULT CALLBACK FMOD_TrackbarProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    PluginParam *param = (PluginParam *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    bool update = false;
...
BOOL 
#endif
CALLBACK FMOD_DlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
#ifdef _WIN64
	...
	switch (msg)
	{
		case WM_INITDIALOG:
		{
            ...
            */
	        ...
	        gOldProgressProc = (WNDPROC)GetWindowLongPtr(GetDlgItem(hwnd, IDC_PROGRESS1), GWLP_WNDPROC);
            SetWindowLongPtr(GetDlgItem(hwnd, IDC_PROGRESS1), GWLP_WNDPROC, (LONG)&FMOD_ProgressWindowProc);
...
BOOL 
#endif
CALLBACK FMOD_OutputSelectProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
#ifdef _WIN64
    result = gSystem->setPluginPath("../../api/plugins/64bit");
#else
