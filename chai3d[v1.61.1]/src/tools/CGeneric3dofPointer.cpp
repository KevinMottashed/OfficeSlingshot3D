...
cGeneric3dofPointer::cGeneric3dofPointer(cWorld* a_world)
{
    ...
    //cProxyGraphPointForceAlgo* default_proxy = new cProxyGraphPointForceAlgo;
	// by gaecha
	cProxyGraphPointForceAlgo* default_proxy = new cProxyGraphPointForceAlgo;
	//
...
cGeneric3dofPointer::~cGeneric3dofPointer()
{
	...
	// by gaecha
	// to release the force algorithm
	    // compute forces in world coordinates for each point force algorithm
    for (int i=0; i<m_pointForceAlgos.size(); i++)
    {
...
void cGeneric3dofPointer::render(const int a_renderMode)
{
    ...
    cProxyGraphPointForceAlgo* default_proxy = getProxy();
...
/*!
      This function searches the vector of point force algorithms and
			returns the first proxy (instance of cProxyGraphPointForceAlgo) that
...
*/
//===========================================================================
cProxyGraphPointForceAlgo* cGeneric3dofPointer::getProxy()
{
	  for (unsigned int i=0; i<m_pointForceAlgos.size(); i++)
		{
		  cProxyGraphPointForceAlgo* default_proxy = dynamic_cast<cProxyGraphPointForceAlgo*>(m_pointForceAlgos[i]);
		  if (default_proxy != NULL)
