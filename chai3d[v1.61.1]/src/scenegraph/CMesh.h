...
// by gaecha for bump
  public:
    // METHODS - VERTICES
	//!
	void setBumpmap(cTexture2D* a_bumpmap, const bool a_affectChildren = false);
	void setBumpmap(unsigned char* a_bumpmap, int a_bumpmaptSizeX, int a_bumpmapSizeY, const bool a_affectChildren = false);
	//!
	void useBumpmap(const bool a_useBumpmap, const bool a_affectChildren = false);
	//!
	cVector3d computePerturbationVector(cTriangle * a_colTriangle, cVector3d & a_proxyPoint);
	//!
	bool m_useBumpmap;
  ...
  protected:
    //! My Bumpmap
    cTexture2D* m_bumpmap;
	//!
	unsigned char * m_bumpmapData;
	//!
	int m_bumpmapSizeX;
	int m_bumpmapSizeY;
	int m_channel;
	//! parameterized collision point s,t
	double m_S0;
	double m_S1;
//
};
