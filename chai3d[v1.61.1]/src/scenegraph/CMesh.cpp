...
/*
void cMesh::setBumpmap(cTexture2D* a_bumpmap, const bool a_affectChildren)
{

  m_bumpmap = a_bumpmap;
  
  m_bumpmapData =  m_bumpmap->m_image_loader.getData();

  m_bumpmapSizeX = m_bumpmap->m_image_loader.getWidth();
  m_bumpmapSizeY = m_bumpmap->m_image_loader.getHeight();
  m_channel = 4;
  ...
  if (a_affectChildren)
  {
      for (unsigned int i=0; i<m_children.size(); i++)
      {
          ...
          if (nextMesh)
          {
              nextMesh->setBumpmap(a_bumpmap, a_affectChildren);
          }
      }
  }
}
...
void cMesh::setBumpmap(unsigned char* a_bumpmap, int a_bumpmaptSizeX, int a_bumpmapSizeY, const bool a_affectChildren)
{
  m_bumpmapData =  a_bumpmap;

  m_bumpmapSizeX = a_bumpmapSizeY;
  m_bumpmapSizeY = a_bumpmapSizeY;
  m_channel = 3;
  ...
  if (a_affectChildren)
  {
      for (unsigned int i=0; i<m_children.size(); i++)
      {
          ...
          if (nextMesh)
          {
              nextMesh->setBumpmap(a_bumpmap, a_bumpmaptSizeX, a_bumpmapSizeY, a_affectChildren);
          }
      }
  }
}

//void cBumpMesh::setParameter(double a_S0, double a_S1)
//{
//	m_S0 = a_S0;
//	m_S1 = a_S1;
//}
...
cVector3d cMesh::computePerturbationVector(cTriangle * a_colTriangle, cVector3d & a_proxyPoint)
{
	cVector3d temp;
	temp.zero();
	if(m_bumpmapData == NULL)
		return temp;
	...
	cVector3d t_proxyPoint = a_proxyPoint;
	t_proxyPoint.sub(m_globalPos);
	transLocalRot.mul(t_proxyPoint);
	

    // Get the position of the triangle's vertices
    cVector3d t_vertex0 = m_vertices[a_colTriangle->getIndexVertex0()].getPos();
    cVector3d t_vertex1 = m_vertices[a_colTriangle->getIndexVertex1()].getPos();
    cVector3d t_vertex2 = m_vertices[a_colTriangle->getIndexVertex2()].getPos();
	...
	cVector3d t_Q = cSub(t_proxyPoint, t_vertex0);
	
	// proxy on to the trianlge
	double height = cDot(t_N, t_Q);
	t_Q = cAdd(t_Q, cMul(-height, t_N));
	...
	//
	cVector3d t_UVvertex0 = m_vertices[a_colTriangle->getIndexVertex0()].getTexCoord();
	t_UVvertex0.z = 0.0;
	cVector3d t_UVvertex1 = m_vertices[a_colTriangle->getIndexVertex1()].getTexCoord();
	t_UVvertex1.z = 0.0;
	cVector3d t_UVvertex2 = m_vertices[a_colTriangle->getIndexVertex2()].getTexCoord();
	t_UVvertex2.z = 0.0;
	t_UVvertex0.x *= m_bumpmapSizeX;
	t_UVvertex0.y *= m_bumpmapSizeY;
	t_UVvertex1.x *= m_bumpmapSizeX;
	t_UVvertex1.y *= m_bumpmapSizeY;
	t_UVvertex2.x *= m_bumpmapSizeX;
	t_UVvertex2.y *= m_bumpmapSizeY;

    cVector3d t_UVE0, t_UVE1;

    // Compute the triangle's normal
    t_UVvertex1.subr(t_UVvertex0, t_UVE0);
    t_UVvertex2.subr(t_UVvertex0, t_UVE1);
	
	cVector3d UV = t_UVvertex0;
	UV.add(cMul(t_S0, t_UVE0));
	UV.add(cMul(t_S1, t_UVE1));

//	int Uindex = (int)UV.x;
//	Uindex = (Uindex+m_bumpmapSizeX*100)%m_bumpmapSizeX;
//	int Vindex = (int)UV.y;
//	Vindex = (Vindex+m_bumpmapSizeY*100)%m_bumpmapSizeY;

	// 텍스쳐가 repeated 하게 붙어 있을 때 uv값이 0보다 작거나 1보다 커진다.
	// 이때 텍스쳐가 seamless하게 이어지려면 거울을 바다보듯 텍스쳐가 붙어 있어야 한다.
	int Uindex = (int)UV.x;
	int Vindex = (int)UV.y;

	Uindex = abs(Uindex)%(2*m_bumpmapSizeX);
	Vindex = abs(Vindex)%(2*m_bumpmapSizeY);

	if(Uindex > m_bumpmapSizeX && Vindex >= 0 && Vindex <= m_bumpmapSizeY) {
		Uindex = 2*m_bumpmapSizeX - Uindex;
	}
	else if(Uindex >= 0 && Uindex <= m_bumpmapSizeX && Vindex > m_bumpmapSizeY) {
		Vindex = 2*m_bumpmapSizeY - Vindex;
	}
	else if(Uindex > m_bumpmapSizeX && Vindex > m_bumpmapSizeY) {
		Uindex = 2*m_bumpmapSizeX - Uindex;
		Vindex = 2*m_bumpmapSizeY - Vindex;
	}

	if(Uindex == m_bumpmapSizeX)
		Uindex = 0;
	if(Vindex == m_bumpmapSizeY)
		Vindex = 0;

	int rightNextUindex, upNextVindex;
	if(Uindex == m_bumpmapSizeX-1)
		rightNextUindex = 0;
	else
		rightNextUindex = Uindex+1;
	if(Uindex == m_bumpmapSizeY-1)
		upNextVindex = 0;
	else
		upNextVindex = 0;
	////////////////////////////////////////////////
	if( m_bumpmapSizeX*m_channel*Vindex + m_channel*Uindex <= m_bumpmapSizeX*m_bumpmapSizeY*m_channel &&
		m_bumpmapSizeX*m_channel*Vindex + m_channel*rightNextUindex <= m_bumpmapSizeX*m_bumpmapSizeY*m_channel &&
		m_bumpmapSizeX*m_channel*upNextVindex + m_channel*Uindex <= m_bumpmapSizeX*m_bumpmapSizeY*m_channel ) {
	unsigned char * pBumpmapDataUV = &m_bumpmapData[m_bumpmapSizeX*m_channel*Vindex + m_channel*Uindex];
	unsigned char * pBumpmapDataU1V = &m_bumpmapData[m_bumpmapSizeX*m_channel*Vindex + m_channel*rightNextUindex];
	unsigned char * pBumpmapDataUV1 = &m_bumpmapData[m_bumpmapSizeX*m_channel*upNextVindex + m_channel*Uindex];

	double Fu = pBumpmapDataU1V[0] - pBumpmapDataUV[0];
	double Fv = pBumpmapDataUV1[0] - pBumpmapDataUV[0];

	cVector3d NPu, NPv;
	NPu.set(0.0, -1.0, 0.0);
	NPv.set(-1.0, 0.0, 0.0);
	cVector3d UVperturbationVector;
	UVperturbationVector.zero();
	UVperturbationVector.add(cMul(Fu, NPv));
	UVperturbationVector.add(cMul(Fv, NPu));


	cVector3d t_UVQ = UVperturbationVector;
	double t_UVQ0 = cDot(t_UVE0,t_UVQ);
	double t_UVQ1 = cDot(t_UVE1,t_UVQ);
	double t_UVE00 = cDot(t_UVE0,t_UVE0);
	double t_UVE01 = cDot(t_UVE0,t_UVE1);
	double t_UVE11 = cDot(t_UVE1,t_UVE1);
	double t_UVD = (t_UVE00 * t_UVE11) - (t_UVE01 * t_UVE01);

	double t_UVS0 = ((t_UVE11 * t_UVQ0) - (t_UVE01 * t_UVQ1)) / t_UVD;
	double t_UVS1 = ((t_UVE00 * t_UVQ1) - (t_UVE01 * t_UVQ0)) / t_UVD;

	cVector3d perturbationVector;
	perturbationVector.zero();
	perturbationVector.add(cMul(t_UVS0, t_E0));
	perturbationVector.add(cMul(t_UVS1, t_E1));

	m_globalRot.mul(perturbationVector);
...
void cMesh::useBumpmap(const bool a_useBumpmap, const bool a_affectChildren)
{
	m_useBumpmap = a_useBumpmap;
    ...
    if (a_affectChildren)
    {
        for (unsigned int i=0; i<m_children.size(); i++)
        {
            ...
            if (nextMesh)
            {                
                nextMesh->useBumpmap(a_useBumpmap, a_affectChildren);
            }
        }
    }
}