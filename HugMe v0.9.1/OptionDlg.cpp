...
BOOL COptionDlg::OnInitDialog() 
{
	...
	for(i=0; i<NUM_IMAGE_DEVICE; i++)
		pImageDevice->InsertString(i, m_pChatDlg->m_pHugMe->m_nameImageDevice[i]);
	...
	for(i=0; i<NUM_INPUT_DEVICE; i++)
		pInputDevice->InsertString(i, m_pChatDlg->m_pHugMe->m_nameInputDevice[i]);
...
void COptionDlg::OnUseImageDevice() 
{
	...
	if( m_bUseImageDevice ) {
		...
		pImageDevice->SelectString(0, m_pChatDlg->m_pHugMe->m_nameImageDevice[m_config.m_typeImageDevice]);
	}
	else
		pImageDevice->EnableWindow(FALSE);
}
...
void COptionDlg::OnUseInputDevice() 
{
	...
	if( m_bUseInputDevice ) {
		...
		pInputDevice->SelectString(0, m_pChatDlg->m_pHugMe->m_nameInputDevice[m_config.m_typeInputDevice]);
		if(m_config.m_typeInputDevice == ONE_FINGER_GLOVE)
...
{
	// TODO: Add your control notification handler code here
	if(m_pChatDlg->m_pHugMe->m_device.m_bOnDisplayDevice) {
		m_pChatDlg->m_pHugMe->m_pDisplayDeviceJacket->test1by1();
	}
}
